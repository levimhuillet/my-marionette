...
Shader "Custom/Proximity" {
	Properties
	{
		...
		_VisibleDistance("Visibility Distance", float) = 15.0 // At what distance is the object fully visible?
	}
		SubShader{
			...
			Pass
			{
				...
				float4 frag(vertexOutput input) : COLOR
				{
					...
					float dist = input.position_in_world_space.y - _PlayerPosition.y; // distance(input.position_in_world_space, _PlayerPosition);

					// Return appropriate transparency
					if (dist >= _VisibleDistance) {
					  return tex2D(_MainTex, float4(input.tex)); // Visible
					   ...
					   tex.a = (dist / _VisibleDistance) * (dist/ _VisibleDistance);
					   return tex;
					}
				}
